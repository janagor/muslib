...
Signal2d frame(const Signal& sig, unsigned frame_length, unsigned hop_length) {
    ...
    for (size_t start = 0; start + frame_length <= sig.size(); start += hop_length) {
        frames.emplace_back(
            sig.begin() + start, sig.begin() + start + frame_length
        );
    }
...
Signal normalized(const Signal& sig) {
    ...
    if (std::abs(norm) < eps) return sig;
